...
const DAILY_API_LIMIT = 100000;    // Your daily API call limit
const SAFETY_BUFFER = 500;         // Stop 500 calls before limit (safety margin)
// Each ETF uses 2 API calls: 1 for fundamentals + 1 for prices
// Max ETFs per run = (limit - buffer) / 2 calls per ETF
const CALLS_PER_ETF = 2;
const MAX_CALLS_PER_RUN = Math.floor((DAILY_API_LIMIT - SAFETY_BUFFER) / CALLS_PER_ETF); // ~49,750 ETFs per run
...
async function loadEtfsWithDailyLimit() {
  ...
  console.log(`  📦 API calls per ETF: ${CALLS_PER_ETF} (fundamentals + prices)`);
  console.log(`  ✅ Max ETFs today: ${MAX_CALLS_PER_RUN} ETFs`);
  ...
  for (let i = 0; i < etfsForToday.length; i++) {
    ...
    try {
      // Step 1: Sync ETF fundamentals (1 API call)
      const synced = await etfService.syncEtf(symbol.Code);
      
      if (synced) {
        // Step 2: Sync price history (1 API call)
        await etfService.syncPrices(symbol.Code);
        todayStats.apiCallsUsed++; // Count the extra price call
        
        // Step 3: Calculate metrics from prices (0 API calls)
        await etfService.calculateMetrics(symbol.Code);

        progress.succeeded++;
        todaySucceeded++;
        console.log(`  ✅ Fundamentals + Prices + Metrics`);
      } else {
        progress.failed++;
        todayFailed++;
