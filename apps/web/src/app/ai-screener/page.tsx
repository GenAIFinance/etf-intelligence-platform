...
// Confidence badge as a component — avoids JSX.Element in module-level constants
  ...
  if (confidence === 'medium') {
    return (
      <div className="flex items-center gap-1.5 text-xs text-amber-600">
        ...
        <span>Medium confidence — some assumptions made</span>
      </div>
    );
  }
  return (
    <div className="flex items-center gap-1.5 text-xs text-red-500">
      <AlertCircle className="w-3.5 h-3.5" />
      <span>Low confidence — please review the settings below</span>
    </div>
  );
}
...
function constraintChips(c: Constraints): string[] {
  ...
  if (c.minAUM !== null) {
    const label = c.minAUM >= 1e9
      ? `$${(c.minAUM / 1e9).toFixed(1)}B`
      : `$${(c.minAUM / 1e6).toFixed(0)}M`;
    out.push(`AUM ≥ ${label}`);
  }
...
function ResultCard({ item, rank, isSelected, onToggle }: {
  ...
  return (
    <div className={`bg-white border rounded-xl overflow-hidden hover:shadow-md transition-all ${
      isSelected ? 'border-teal-400 ring-1 ring-teal-300' : 'border-gray-200 hover:border-teal-300'
    }`}>
      <div className="p-4">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-red-500">
                {item.maxDrawdown !== null ? `-${fmt(item.maxDrawdown, 'pct2')}` : 'N/A'}
              </span>
            </div>
            <div className="flex justify-between">
...
function ConfirmationCard({ parsed, onConfirm, onEditConfirm }: {
  ...
  const safeFloat = (s: string): number | null => {
  ...
  const activeChips = constraintChips(parsed.request.constraints);
        ...
        <div className="px-4 py-3 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setDraft(d => ({ ...d, objective: e.target.value as Objective }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setDraft(d => ({ ...d, riskProfile: e.target.value as RiskProfile }))
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="number" placeholder="e.g. 0.50" min="0" max="5" step="0.01"
                value={draft.constraints.maxExpenseRatio !== null ? (draft.constraints.maxExpenseRatio * 100).toFixed(2) : ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setC('maxExpenseRatio', e.target.value ? safeFloat(e.target.value)! / 100 : null)
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
            </div>
            <div>
              ...
              <input
                type="number" placeholder="e.g. 1" min="0" step="0.1"
                value={draft.constraints.minAUM !== null ? (draft.constraints.minAUM / 1e9).toFixed(1) : ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setC('minAUM', e.target.value ? safeFloat(e.target.value)! * 1e9 : null)
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="number" placeholder="e.g. 0.5" step="0.1" min="0"
                value={draft.constraints.minSharpe ?? ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setC('minSharpe', e.target.value ? safeFloat(e.target.value) : null)
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
            </div>
            <div>
              ...
              <input
                type="number" placeholder="e.g. 20" min="0" max="100" step="1"
                value={draft.constraints.maxVolatility !== null ? (draft.constraints.maxVolatility * 100).toFixed(0) : ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setC('maxVolatility', e.target.value ? safeFloat(e.target.value)! / 100 : null)
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="number" placeholder="e.g. 10" step="1"
                value={draft.constraints.minReturn3Y !== null ? (draft.constraints.minReturn3Y * 100).toFixed(0) : ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setC('minReturn3Y', e.target.value ? safeFloat(e.target.value)! / 100 : null)
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
            </div>
            <div>
              ...
              <input
                type="number" placeholder="e.g. 8" step="1"
                value={draft.constraints.minReturn5Y !== null ? (draft.constraints.minReturn5Y * 100).toFixed(0) : ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setC('minReturn5Y', e.target.value ? safeFloat(e.target.value)! / 100 : null)
                }
                className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
              />
            </div>
          ...
          <div>
            ...
            <select
              value={draft.constraints.esgPreference}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setC('esgPreference', e.target.value as EsgPref)
              }
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400"
            >
          ...
          <div>
            ...
            <input
              type="text" placeholder="e.g. oil, tobacco, weapons"
          ...
          <div className="flex gap-2 pt-1">
            <button
              onClick={() => { onEditConfirm(draft); setEditMode(false); }}
              className="flex-1 py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold rounded-xl transition-colors"
            >
...
export default function AiScreenerPage(): React.ReactElement {
  ...
  async function parseQuery(q: string, previous?: ScreenerRequest, hint?: FeedbackHint): Promise<void> {
    setError(null);
    setMode('parsing');
    try {
  ...
  async function runScreener(req: ScreenerRequest): Promise<void> {
    setError(null);
    setMode('screening');
    setActiveReq(req);
    setResults(null);
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4 space-y-3 mt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {([
                  ...
                  <button
                    key={hint}
                    onClick={() => void handleFeedbackChip(hint)}
