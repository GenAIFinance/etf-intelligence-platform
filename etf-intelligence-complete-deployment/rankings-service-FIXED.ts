...
export class RankingsService {
  ...
  async getTop10Rankings(investmentAmount: number = 10000): Promise<RankingsResponse> {
    ...
    // Fetch all ETFs with necessary fields
  ...
  private getHighestSavings(
    ...
    return etfs
      ...
      .filter(e => e.savingsUSD > 0) // Only show ETFs cheaper than category median
  ...
  private getMostDiversified(etfs: any[]): RankingItem[] {
    return etfs
      .filter(e => e.holdingsCount && e.holdingsCount > 0)
      ...
      .map((etf, index) => ({
        ...
        ticker: etf.ticker,
        name: etf.name,
        value: etf.holdingsCount!,
        formattedValue: `${etf.holdingsCount!.toLocaleString()} holdings`,
      }));
  }
